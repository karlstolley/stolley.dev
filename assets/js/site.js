...
(function() {
  ...
  // Human-friendly days and months
    ...
    if (github_anchor !== null) {
      ...
      if(typeof(github_url) !== "undefined") {
        ...
        .then(function(data) {
          ...
          commit.html = document.createElement('time');
          commit.html.datetime = commit.stamp;
          commit.html.innerHTML =
            'Last updated on ' + commit.time_string + ' to ' +
            '<a id="commit-message" href="' + commit.url + '">' + escapeHTML(commit.message) + '</a>. ' +
            github_anchor.outerHTML;
          // Append HTML after the original GitHub anchor
          document.querySelector('#github').insertAdjacentElement('afterend', commit.html);
          // Remove the original GitHub anchor (re-inserted above in `commit.html.innerHTML`)
...
/* PrismJS 1.29.0
https://prismjs.com/download.html#themes=prism-tomorrow&languages=markup+css+clike+javascript+bash+git+json+ruby */
